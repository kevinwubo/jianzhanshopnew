    ...
    public partial class article
    {
        ...
        public Model.article GetModel(int id)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    model.content = model.content;//.Replace(@"/upload", "http://116.62.124.214/upload");
                }
        ...
        /// <returns></returns>
        public DataSet GetArticleList(string sqlwhere ,int count)
        {
            string Top="";
            if (count > 0)
            {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT "+Top+" a.id,a.title as articleTitle,a.Call_Index,a.img_url,zhaiyao,b.title,a.add_time ");
            strSql.Append("FROM dbo.dt_article a left join dt_article_category b on a.category_id=b.id");
            //if (sqlwhere != "")
            {
                strSql.Append(" where 1=1 and b.title!='建盏天下' " + sqlwhere);
            }
            strSql.Append(sqlwhere + " order by   a.add_time  desc ");
            return DbHelperSQL.Query(strSql.ToString());
        }
        ...
        /// <param name="PlatePosition"></param>
        /// <returns></returns>
        public DataSet GetArticleList(string sqlwhere, int count, int indexCount)
        {
            string OrderBy = " a.add_time  desc ";
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT top " + count + " a.id,a.title as articleTitle,a.Call_Index,a.img_url,zhaiyao,b.title,a.add_time ");
            strSql.Append("FROM dbo.dt_article a left join dt_article_category b on a.category_id=b.id");
            strSql.Append(" where a.id not in ");
            strSql.Append(" (SELECT top " + indexCount + " a.id FROM dbo.dt_article a left join dt_article_category b on a.category_id=b.id where 1=1 " + sqlwhere + " order by " + OrderBy + " ) ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT a.id,a.title as articleTitle,a.img_url,zhaiyao,b.title,a.add_time ");
            strSql.Append("FROM dbo.dt_article a ");
            strSql.Append("left join dt_article_category b on a.category_id=b.id ");
            if (sqlwhere != "")
            {
