...
{
    public partial class Codes : Web.UI.ManagePage
    {
        ...
        private void RptBind(string _strWhere, string _orderby)
        {
            ...
            string pageUrl = Utils.CombUrlTxt("Codes.aspx", "keywords={0}&page={1}", this.keywords, "__id__");
            PageContent.InnerHtml = Utils.OutPageList(this.pageSize, this.page, this.totalCount, pageUrl,8);
        }

        protected void btnSearch_Click(object sender, EventArgs e)
        {
            string sqlwhere = " IsShow=" + ddlStatus.SelectedValue + "";
            this.pageSize = GetPageSize(40); //每页数量
            RptBind(sqlwhere, "  sort ");
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == DTEnums.ActionEnum.Edit.ToString())
            {
                model.ID = this.id;
                bll.Update(model);
                JscriptMsg("更新成功！", "Codes.aspx", "Success");
            }
            else
            {
                ...
                {
                    bll.Add(model);
                    JscriptMsg("添加成功！", "Codes.aspx", "Success");
                }
                else
                {
                    JscriptMsg("已经存在、不能重复添加！", "Codes.aspx", "Success");
                }
            }
        }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！", "Codes.aspx", "Success");
        }
    }
}