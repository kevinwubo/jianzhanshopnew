<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="xiaojiangzhuanchang.aspx.cs" Inherits="DTcms.Web.activity.xiaojiangzhuanchang" %>
...
<head>
    <meta charset="utf-8">
    <meta content="小匠拍卖专场" http-equiv="keywords">
    <meta name="description" content="小匠拍卖专场">
...
<div id="guanzhuweixin-s" class="flex-row bg_color_f navheight" style="align-items: center;max-width: 750px">
    ...
    <div class="flex-col texta-c px36 color_57">小匠拍卖专场</div>
...
<img class="w100" style="display: block" src="static/images/315/01.jpg"/>
               ...
               <% if(IsShowTiXing)
                ...
                <div class="flex-row  mar-l-40 mar-r-40 mar-t-16" style="align-items: center">
                    <div class="px24">当前价：</div>
                     ...
                     style="background: #ffc513;height: .5rem;color: white;line-height: .5rem" onclick="alertTiXing('<%=model.ProductID.ToString() %>','<%=model.ProductName %>','<%=model.Author %>','小匠')">
                    我要提醒
                </div>
                <%}else
                { %>
                ...
                { %>
                <div class="w100 texta-c mar-t-16"
                     style="background: #ffc513;height: .5rem;color: white;line-height: .5rem" onclick="alertChuJia('<%=model.ProductID.ToString() %>','<%=model.ProductName %>','<%=model.Author %>','小匠','<%=model.CustomerOffer %>','<%=model.HighTelePhone %>','<%=model.MarketPrice%>')">
                    立即出价
                </div>
                <%} %>
                <%} %>
...
<div class="w100 flex-row" style="position: fixed;height: 0.7rem;bottom:-1px;border-top: 1px solid #fff;max-width: 750px">
    ...
    <a class="flex-col b_itm" href="xiaojiangtulu.aspx?source=<%=Source %>">去图录页</a>
</div>
...
<script>
    $("#btnSearch").bind("click", function (e) {
        ...
        window.location.href = "xiaojiangzhuanchang.aspx?source=" + source + "&keyword=" + keyword;
    });
    $('.M-box').pagination({
        mode: 'fixed'
    });
