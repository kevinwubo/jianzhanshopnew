...
+ function($) {
    ...
    $.prompt = function(text, title, onOK, onCancel, input) {
        ...
        var modal = $.modal({
            text: '<p class="weui-prompt-text">'+(config.text || '')+'</p><textarea type="text" class="weui-input weui-prompt-input" id="weui-prompt-input" rows="4"  value="' + (config.input || '') + '" />',
            title: config.title,
            autoClose: false,
            buttons: [
                {
...
+function ($) {
    ...
    PTR.prototype.touchStart = function(e) {
        if(this.container.hasClass("refreshing")||$(document).scrollTop()>0) return;
    ...
    PTR.prototype.touchEnd = function() {
        this.start = false;
        if(this.diffY <= 0 || this.container.hasClass("refreshing")||$(document).scrollTop()>0) return;
